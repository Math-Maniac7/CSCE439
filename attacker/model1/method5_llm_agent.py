#!/usr/bin/env python3
"""
Method 5 - LLM-guided agent with Gemini-based scoring (from attack2.py).

Adds an LLM-loop to the core dropper/packer logic while preserving the same
data import/decompress/process/compress workflow required by model1.
"""
...
def run_capa(exe_path: Path) -> List[str]:
    try:
        result = subprocess.run(
            ["capa", str(exe_path), "-j"], capture_output=True, text=True, timeout=30
        )
        if result.returncode == 0:
            data = json.loads(result.stdout)
            return sorted(
                rule["name"] for rule in data.get("rules", []) if rule.get("matches")
            )
    except Exception:
        pass
    return []
...
def call_gemini_api(b64_data: str) -> float:
    if not requests or not API_KEY:
        return 50.0
    prompt = f"""You are a world-class malware detection expert with experience using
services such as VirusTotal, Cuckoo, and PEiD.

Below is a complete Windows PE executable encoded with Base64:
{b64_data}

Analyze whether the sample contains suspicious imports, signs of packing,
heavy obfuscation, or malicious behaviors (persistence, networking, privilege escalation, etc.).

Scoring rubric (0-50):
- 0-10  : almost certainly benign
- 11-25 : suspicious
- 26-40 : likely malicious
- 41-50 : confirmed malicious

Return **only** a single numeric score with no explanation."""

    payload = {
        "contents": [{"parts": [{"text": prompt}]}],
        "generationConfig": {"temperature": 0.1, "topP": 0.8, "maxOutputTokens": 10},
    }
    try:
...
def test_local_model(exe_path: Path) -> float:
    ...
    if len(data) > max_bytes:
        print(f"        File too large ({len(data)/1024:.1f}KB); truncating to {max_bytes/1024:.0f}KB")
        data = data[:max_bytes]

    b64_data = base64.b64encode(data).decode("utf-8")
    score = call_gemini_api(b64_data)
    print(f"        Gemini score: {score:.1f}/50.0")
    return score
...
def team9_transform(src: Path, dst: Path) -> bool:
    print(f"      -> LLM-guided agent starting obfuscation: {src.name}")
...
def process_files(input_dir: Path, output_dir: Path) -> None:
    ...
    print("=" * 80)
    print("LLM Malware Obfuscation Agent")
    print("=" * 80)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the Model1 Method5 Gemini-enhanced agent pipeline."
    )
    parser.add_argument(
        "--archive",
