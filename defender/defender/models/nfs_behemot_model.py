...
class PEAttributeExtractor():
    ...
    def extract(self):
        ...
        self.attributes.update({
            ...
            "machine": int(self.lief_binary.header.machine),
            # TODO: NFS only
        ...
        self.attributes.update({
            ...
            "magic": int(self.lief_binary.optional_header.magic),
...
class NFSBehemotModel(object):
    # slugs for each classifier (complement of file name)
    # nfs_classifiers_slugs = ["decisiontree", "knn_3", "randomforest_100", "svc_linear"]
    CLASSIFIER = "randomforest_100"

    # feature extractor used
    nfs_extractor = os.path.dirname(__file__) + '/nfs_behemot/nfs_extractor_tfidf.pkl'
    # scaler used
    nfs_scaler = os.path.dirname(__file__) + '/nfs_behemot/nfs_scaler_minmax.pkl'
    # base name of classifiers
    nfs_clf_base_name = os.path.dirname(__file__) + "/nfs_behemot/nfs_classifier_{}.pkl"

    def __init__(self, thresh: float = 0.1234, name: str = 'NFS Behemot {}'):
        name = name.format(self.CLASSIFIER)
        # get classifier file name
        clf_file = self.nfs_clf_base_name.format(self.CLASSIFIER)
        print("CLF:", clf_file)
        # load model
        self.model = pickle.load(open(clf_file, "rb"))
        self.thresh = thresh
        self.__name__ = name

    def predict(self, bytez: bytes) -> int:
        print("SHA256:", hashlib.sha256(bytez).hexdigest())
        print("Attribute + Feature Extraction")
        start = time.time()
        try:
            # extract features
            # pe_fe_extractor = PEFeatureExtractor(bytez, self.nfs_extractor, self.nfs_scaler)
            fe = PEFeatureExtractor(bytez, self.nfs_extractor, self.nfs_scaler)
            pe_features = fe.extract_features()
        except (pefile.PEFormatError, lief.bad_format, lief.read_out_of_bound) as e:
            print("Error:", e)
            return(1)
        # extract features
        end = time.time()
        ...
        prediction = self.model.predict(pe_features)[0]
        end = time.time()
        print("Elapsed time:", end - start)
        print("PRED", prediction)
        return int(prediction)  # always predict malware (0 for benignware)
